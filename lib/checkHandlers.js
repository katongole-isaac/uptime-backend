...
handlers._checks.post = (data, callback) => {
  ...
  _data.read(tokenDir, token, (err, tokenData) => {
    ...
    _data.read(usersDir, userPhone, (err, userData) => {
      ...
      // check if the url given has dns entries
      const parsedUrl = nodeUrl.parse(`${protocol}://${url}`, true);

      // resolving the given url
      dns.resolve(parsedUrl.hostname, (err, records) => {
        if (err)
          return callback(400, {
            error: `Couldn't resolve given hostname ${parsedUrl.hostname}`,
          });
...
handlers._checks.get = (data, callback) => {
  ...
  //look up checkId
  performance.mark("getting ready to read");
     ...
     "done reading"
      ...
      callback(200, checkData);
       performance.getEntriesByType("measure").forEach((measurement) => {
         debug(`\x1b[33m%s %d\x1b[0m`, measurement.name, measurement.duration);
       });
